...
def _fila_html(ev):
    c = COLORES.get(ev["unidad"], {}).get("line", "#64748B")
    ...
    return (
        ...
        f'<span style="background:{c}1A;color:{c};font-weight:700;font-size:0.7rem;'
        f'padding:2px 9px;border-radius:5px">{ev["unidad"]}</span></td>'
        f'<td style="padding:7px 12px;border-bottom:1px solid #F1F5F9;white-space:nowrap">'
...
def render_bitacora_auto(s, e):
    st.markdown('<div class="sec">Bitácora automática · movimientos por unidad</div>',
    ...
    # ── Estado de limitaciones activas (pendientes) sobre las unidades ──────────
    lim_activas = pd.DataFrame()
    if df_l is not None and not df_l.empty:
        con_unidad = df_l["id_unidad"].apply(
            lambda x: pd.notna(x) and int(float(x)) in ID_UNIDAD_LABEL)
        lim_activas = df_l[(df_l["status"].astype(str).str.lower() == "pendiente") & con_unidad]
    ...
    if lim_activas.empty:
        ...
        chips = "".join(
            ...
            f'{ID_UNIDAD_LABEL.get(int(float(r["id_unidad"])), "")}: '
    ...
    # ── Consolidación cronológica de eventos ────────────────────────────────────
    eventos = []
    _eventos_sscc(df_s, eventos)
    _eventos_despacho(df_d, eventos)
    _eventos_limitaciones(df_l, eventos)

    if not eventos:
        st.caption("Sin movimientos registrados para el período seleccionado.")
        return

    eventos.sort(key=lambda ev: (pd.isna(ev["dt"]), ev["dt"]), reverse=True)

    MAX = 40
    filas = "".join(_fila_html(ev) for ev in eventos[:MAX])
    hdr = "".join(
        ...
        for h in ("Fecha y hora", "Unidad", "Tipo", "Descripción"))
    st.markdown(
        ...
        unsafe_allow_html=True)
    if len(eventos) > MAX:
        st.caption(f"Mostrando los {MAX} movimientos más recientes de {len(eventos)} en el período.")
